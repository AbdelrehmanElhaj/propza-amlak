...
### `sa_call_center` — Smart Call Center

Inbound and outbound calls linked to customers, CRM leads, maintenance requests, and tenancy contracts, plus a lightweight ticketing layer and a basic KPI report. See [Call Center Integration](#call-center-integration) below for the full setup.

**Models**

| Model | Purpose |
|-------|---------|
| `sa.call.center.call` | Unified log of every call (direction, state, wait/talk duration, recording URL, linked partner/ticket) |
| `sa.call.center.ticket` | Lightweight follow-up ticket created from a call |
| `sa.call.center.queue` | Queue definition + members (routing not yet wired — see below) |
| `sa.telephony.gateway` | Provider-agnostic abstract model — phone→partner reverse lookup (reuses `sa.messaging.gateway._normalize_phone`) |
| `sa.telephony.twilio.service` | Live Twilio Voice adapter (webhook signature validation, Access Token generation, `originate_call`) |
| `sa.telephony.asterisk.service` | Secondary/reference Asterisk ARI adapter |

**Key features**
- Real-time browser softphone (Twilio Voice JS SDK) — agents answer/place calls without leaving Odoo
- Click-to-dial "Call" button (`sa_call_center_phone` widget) on partner and CRM lead phone/mobile fields
- Automatic phone → `res.partner` matching on every inbound call
...
The `sa_call_center` module integrates **Twilio Voice** for both inbound and outbound calling, entirely inside the browser (WebRTC) — no desk phone or softphone app required.

### How it works

**Inbound:** a call to your Twilio number hits `POST /callcenter/webhook/twilio/voice`, which rings every user flagged `is_call_center_agent` (via the Twilio Voice JS SDK registered in their browser) in parallel with a fixed forward number — first to answer gets the call. Status updates arrive at `POST /callcenter/webhook/twilio/status`.

**Outbound:** clicking the "Call" button next to a phone number (partner form, CRM lead form) dispatches a `CALL_CENTER:DIAL` event that the browser softphone picks up and connects via `Device.connect()`. Twilio then requests TwiML from `POST /callcenter/webhook/twilio/outbound-voice`, which creates the call record and bridges the agent to the destination number. Status updates land on `POST /callcenter/webhook/twilio/outbound-status/<call_id>`.

Every webhook validates Twilio's `X-Twilio-Signature` (HMAC-SHA1) against a configured public base URL — signature validation is **not** based on `request.httprequest.url`, since `proxy_mode` may not reflect the real external host.

### Twilio setup

Configure at: **Settings → مركز الاتصال (Call Center)**

| Setting | Purpose |
|---------|---------|
| `sa_call_center.telephony_provider` | `twilio` / `asterisk` / `disabled` |
| `sa_call_center.twilio_account_sid` / `twilio_auth_token` | Account credentials |
| `sa_call_center.twilio_phone_number` | Twilio number, E.164 (`+19999999999`) |
| `sa_call_center.twilio_forward_number` | Fixed fallback number for every inbound call |
| `sa_call_center.twilio_public_base_url` | Public URL of this Odoo, as Twilio sees it — used for signature validation |
| `sa_call_center.twilio_api_key_sid` / `twilio_api_key_secret` | API Key (Console → Account → API keys & tokens) — used to mint browser Access Tokens |
| `sa_call_center.twilio_twiml_app_sid` | **Required for outbound calling** — see below |

**Enabling outbound click-to-dial (one-time):**
1. Twilio Console → Voice → TwiML → TwiML Apps → create a new app.
2. Set its **Voice Request URL** to `<public base url>/callcenter/webhook/twilio/outbound-voice`, method **POST**.
3. Copy the app's SID (`AP...`) into `sa_call_center.twilio_twiml_app_sid`.

Without this, Access Tokens only grant `incoming` and outbound `Device.connect()` calls fail with Twilio error 31002 ("Token does not allow outgoing calls").

> Any out-of-band write to these `ir_config_parameter` keys (raw SQL, or an `odoo shell` session separate from the running server) requires a container restart to take effect — the running process keeps serving cached values otherwise, even when the write itself went through the ORM.
...
│   │   ├── models/
│   │   │   ├── telephony_gateway.py     ← provider-agnostic router (sa.telephony.gateway)
