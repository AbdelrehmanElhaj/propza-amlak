...
owner5 = env['res.partner'].create({
    'name':              'شركة الدرعية القابضة',
    'company_type':      'company',
    'is_property_owner': True,
    'sa_cr_number':      '1010987654',
    'sa_iban':           'SA9720000007890123456789',
    'phone':             '+966116543210',
    'email':             'info@diriyah-holdings.sa',
    'street':            'حي الدرعية',
    'city':              'الرياض',
    'country_id':        SA.id,
    'website':           'https://diriyah-holdings.sa',
})
...
tenant11 = env['res.partner'].create({
    'name':           'شركة البستان للتأجير التجاري',
    'company_type':   'company',
    'is_tenant':      True,
    'sa_cr_number':   '4030112233',
    'phone':          '+966505100011',
...
tech4 = env['res.partner'].create({
    'name':              'مجموعة النخبة للصيانة الكهربائية',
    'company_type':      'company',
    'is_technician':     True,
    'sa_cr_number':      '1010876543',
    'sa_hourly_rate':    120.0,
    'sa_call_out_fee':   60.0,
    'sa_response_hours': 3,
    'sa_skill_ids':      [(6, 0, (electrical_skill + ac_skill).ids)],
    'phone':             '+966114503000',
    'email':             'support@elite-maintenance.sa',
    'country_id':        SA.id,
})

tech5 = env['res.partner'].create({
    'name':              'شركة الرازي للسباكة والتدفئة',
    'company_type':      'company',
    'is_technician':     True,
    'sa_cr_number':      '1010765432',
    'sa_hourly_rate':    90.0,
    'sa_call_out_fee':   45.0,
    'sa_response_hours': 4,
    'sa_skill_ids':      [(6, 0, plumbing_skill.ids)],
    'phone':             '+966114503111',
    'email':             'info@razi-plumbing.sa',
    'country_id':        SA.id,
})
...
# ══════════════════════════════════════════════════════════════════════════
# 5 — العقارات  (Properties — 14 units)
# ══════════════════════════════════════════════════════════════════════════
...
prop13 = prop({
    'name':                'فيلا النسيم – ١٥',
    'flat_name':           'حي النسيم، شارع الأمير محمد بن فهد',
    'description':         'فيلا عائلية فاخرة مطلة على حديقة مشتركة مع مسبح صغير خاص.',
    'property_type':       'residential',
    'sa_property_subtype': 'villa',
    'owner_partner_id':    owner5.id,
    'rent_amount':         72000.0,
    'deposit_amount':      72000.0,
    'currency_id':         SAR.id,
    'sa_region_id':        r_dammam.id,
    'sa_city_id':          c_dammam.id,
    'sa_district':         'حي النسيم',
...
prop14 = prop({
    'name':                'محل العليا التجاري – ١٢',
    'flat_name':           'مجمع العليا التجاري، الوحدة ١٢',
    'description':         'محل تجاري مجهز بالكامل في قلب منطقة العليا، مثالي للمطاعم أو البيع بالتجزئة.',
    'property_type':       'commercial',
    'sa_property_subtype': 'shop',
    'owner_partner_id':    owner5.id,
    'rent_amount':         65000.0,
    'deposit_amount':      65000.0,
    'currency_id':         SAR.id,
    'sa_region_id':        r_riyadh.id,
    'sa_city_id':          c_riyadh.id,
    'sa_district':         'حي العليا',
...
# ══════════════════════════════════════════════════════════════════════════
# 6 — عقود الإيجار  (Tenancies — 11 contracts)
# ══════════════════════════════════════════════════════════════════════════
...
# ع١٠ — فيلا النسيم: عقد سكني قائم في الدمام
t10 = tenancy({
    'property_id':         prop13.id,
    'partner_id':          tenant11.id,
    ...
    'ejar_payment_schedule': 'monthly',
    'tenant_id_type':      'national_id',
    'tenant_national_id':  '2098765431',
    'sa_broker_id':        broker3.id,
})

# ع١١ — محل العليا التجاري: عقد تجاري قائم
t11 = tenancy({
    'property_id':         prop14.id,
    'partner_id':          tenant5.id,
    'start_date':          str(ago(10)),
    'end_date':            str(ahead(355)),
    'duration':            12,
    'interval_type':       'months',
    'rent_amount':         65000.0,
    'deposit_amount':      65000.0,
...
# ══════════════════════════════════════════════════════════════════════════
# 9 — طلبات الصيانة وأوامر العمل  (Maintenance — 10 requests, 5 WOs)
# ══════════════════════════════════════════════════════════════════════════
...
req9 = env['sa.maintenance.request'].create({
    'property_id':        prop10.id,
    'description':        'انقطاع التيار الكهربائي في المحل التجاري مع شرر عند اللوحة الرئيسية.',
    'request_date':       str(today),
    'category':           'electrical',
    'priority':           '3',
    'supplier_partner_id':tech4.id,
    'scheduled_date':     str(dt(ahead(2), 10)),
    'estimated_duration': 2.5,
    'labor_cost':         300.0,
    'materials_cost':     250.0,
    'cost_bearer':        'owner',
    'notes':              'تحقق من الفيوز ولوحة التوزيع الرئيسية.',
})
req9.action_approve()
req9.action_schedule()

req10 = env['sa.maintenance.request'].create({
    'property_id':        prop11.id,
    'description':        'تسرب في أنابيب المياه في المحل التجاري، يحتاج إلى استبدال سريع لمنع تلف المخزون.',
    'request_date':       str(ago(1)),
    'category':           'plumbing',
    'priority':           '2',
    'supplier_partner_id':tech5.id,
    'scheduled_date':     str(dt(ahead(3), 12)),
    'estimated_duration': 4.0,
    'labor_cost':         280.0,
    'materials_cost':     320.0,
    'cost_bearer':        'owner',
})
req10.action_approve()

wo5 = env['sa.maintenance.work_order'].create({
    'request_id':       req9.id,
...
# ══════════════════════════════════════════════════════════════════════════
# 16 — حجوزات CRM  (CRM Reservations — 4)
# ══════════════════════════════════════════════════════════════════════════
print("إنشاء بيانات الحجوزات... (Creating CRM reservations...)")

def reservation(vals):
    return env['sa.crm.reservation'].create(vals)

res1 = reservation({
    'lead_id':        lead4.id,
    'property_id':    prop8.id,
    'user_id':        _user_agent.id,
    'date_start':     str(today),
    'date_end':       str(ahead(7)),
    'deposit_amount': 5000.0,
    'currency_id':    SAR.id,
    'notes':          'حجز أولي للشقة في الدمام لإبقاء العرض متاحًا أثناء التفاوض.',
})
...
res2 = reservation({
    ...
    'user_id':        _user_agent.id,
    'date_start':     str(today),
    'date_end':       str(ahead(10)),
    'deposit_amount': 12000.0,
    'currency_id':    SAR.id,
    'notes':          'حجز تجاري فعال للمكتب بعد الجولة الأولى.',
})
res2.action_activate()

res3 = reservation({
    'lead_id':        lead5.id,
    'property_id':    prop12.id,
    'user_id':        _user_manager.id,
    'date_start':     str(ago(40)),
    'date_end':       str(ago(5)),
    'deposit_amount': 8000.0,
    'currency_id':    SAR.id,
    'notes':          'حجز مستودع انتهى تلقائياً بعد رفض الطلب.',
    'state':          'expired',
})
...
res4 = reservation({
    ...
    'property_id':    prop8.id,
    'user_id':        _user_manager.id,
    'date_start':     str(ago(10)),
    'date_end':       str(ahead(5)),
...
print(f"  الملاك           / Owners:      5  ({owner1.name[:20]}…، {owner5.name[:20]}…)")
print(f"  المستأجرون       / Tenants:    11  (٧ موثَّق، ٢ قيد المراجعة، ٢ مسودة)")
print(f"  الوسطاء          / Brokers:     3")
print(f"  الفنيون          / Technicians: 5")
print(f"  العقارات         / Properties: 14  (فلل + شقق + مكاتب + مستودع + محل)")
print(f"  عقود الإيجار     / Tenancies:   11  (٨ نشطة، ١ مؤكد، ٢ مسودة)")
print(f"  الدفعات          / Payments:   {env['sa.rent.payment'].search_count([])}")
print(f"  المعاينات        / Inspections: 5  (٣ موقعة، ١ مكتملة، ١ مسودة)")
print(f"  طلبات الصيانة   / Maint Reqs:  10  (متنوعة الحالات)")
print(f"  أوامر العمل     / Work Orders: 5  (٢ مجدول، ٢ منجز، ١ قيد التنفيذ)")
print(f"  عقود الصيانة    / Maint Conts: 2  (تكييف + سباكة، نشطة)")
print(f"  حجوزات CRM      / Reservations: {env['sa.crm.reservation'].search_count([])}  (مسودة، محجوزة، منتهية، تحويل)")
